...
export function getNucleusRepository(): NucleusRepository {
    return {
        ...
        createNucleus: async (data) => {
            ...
            const result = await response.json();
            if (!response.ok) {
                throw new Error(result.error || result.message || 'Error creating nucleus');
            }
            const nucleus: NucleusInfoDAO = result.data;
        ...
        updateNucleus: async (id, data) => {
            ...
            const result = await response.json();
            if (!response.ok) {
                throw new Error(result.error || result.message || 'Error updating nucleus');
            }
            const nucleus: NucleusInfoDAO = result.data;
            return daoToNucleusModel(nucleus);
        },
        deleteNucleus: async (id: string) => {
            ...
            if (!response.ok) {
                ...
                throw new Error(result?.error || result?.message || 'Error deleting nucleus');
            }
        }
    };
}
